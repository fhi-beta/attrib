...
We can see in the above dataset that the colums *id*, *sim_id*,  *observed_value*, *sim_value_temperature_high=0*, *sim_value_pr100_ili_lag_1=0* are added to the previous set of colums. For each row in the original dataset we now have 500 
...
We proceed by aggregating the county dataset to the national/seasonal level. Afterwards we calculate the expected attributable mortality, `exp_attr`, by substracting `value` (the simulated expected number of deaths given the reference value of the exposure) from *the observed_value*.
...
data.table::setkeyv(
  ...
  col_names[!col_names %in% c("exp_irr", "sim_id", "observed_value", "sim_value_pr100_ili_lag_1=0")]
)
