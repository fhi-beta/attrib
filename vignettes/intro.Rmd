...
aggregated_county_to_nation_weekly_irr <- aggregated_county_to_nation_sim_irr[,
              unlist(recursive = FALSE, lapply(.(median = median, q05 = q05, q95 = q95),
                                               function(f) lapply(.SD, f)
              )), 
              by=eval(data.table::key(aggregated_county_to_nation_sim_irr)),
              .SDcols = c("exp_irr")]
aggregated_county_to_nation_weekly_irr[, tag := "aggregated"]

aggregated_county_to_nation_weekly_irr
```

Now we compare the resulting values for irr with the ones obtained by coef(fit_county).
```{r}
coef_fit_county <- as.data.table(coef(fit_county)$season)
col_names_coef <- c("pr100_ili_lag_1")
irr_data <- coef_fit_county[, ..col_names_coef]
irr_data[, irr := exp(pr100_ili_lag_1)]
irr_data[, q05 := exp(pr100_ili_lag_1 - 1.96 *0.003761)]  # 0.003761 is the standard deviation from coef(fit_county)
irr_data[, q95 := exp(pr100_ili_lag_1 + 1.96 *0.003761)]
irr_data[, tag := "from_coef"]
irr_data
```

```{r}
irr_data <- cbind(season = aggregated_county_to_nation_weekly_irr$season, irr_data)
irr_data
```
```{r}
total_data <- rbindlist(list(irr_data, aggregated_county_to_nation_weekly_irr), use.names = FALSE)
total_data[, pr100_ili_lag_1 := NULL]
total_data
```
As one can see these valueas are very similar.

The benefit of the aggregated aproach is that this prosess will be equally easy no matter the complecity of what one wants to compute the irr for. 
One does not have ot take the variance covariance matrix in to account at any stage. 



```{r}
q <- ggplot(data = total_data, 
